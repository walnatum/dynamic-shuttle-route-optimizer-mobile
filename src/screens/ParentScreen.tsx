...
//   const apiKey = process.env.REACT_NATIVE_GRAPHHOPPER_API_KEY || "53e2b16a-00cb-425b-b216-6483d8baa12d";
...
//   const getPlaceCoordinates = async (query: string) => {
//     const geocodeUrl = `https://graphhopper.com/api/1/geocode?q=${encodeURIComponent(query)}&key=${apiKey}&limit=1`;
//     try {
//       const response = await fetch(geocodeUrl);
//       const data = await response.json();
//       if (data.hits && data.hits.length > 0) {
//         const { point, name } = data.hits[0];
//         return {
//           name: name || query,
//           latitude: point.lat,
//           longitude: point.lng,
...
//     const routeUrl = `https://graphhopper.com/api/1/route?point=${startCoords.lat},${startCoords.lng}&point=${endCoords.lat},${endCoords.lng}&vehicle=car&key=${apiKey}&type=json&points_encoded=true`;
//     try {
//       const response = await fetch(routeUrl);
//       const data = await response.json();
//       if (data.paths && data.paths.length > 0) {
//         points = decodePolyline(data.paths[0].points);
//         const durationSeconds = data.paths[0].time / 1000;
//         const durationText = formatDuration(durationSeconds);
//         setRouteCoordinates(points);
//         setParkingMarkers([]);
//         setTravelTimes([durationText]);
//         mapRef.current?.fitToCoordinates(points, {
//           edgePadding: { top: 50, right: 50, bottom: 50, left: 50 },
//         });
//         timesByMode["driving"] = durationText;
//       } else {
...
//     if (minutes > 0 || hours === 0) result += `${minutes} min${minutes !== 1 ? "s" : ""}`;
...
//         fetch(
//           `https://graphhopper.com/api/1/geocode?point=${latitude},${longitude}&reverse=true&key=${apiKey}`
//         )
//           .then((response) => response.json())
//           .then((data) => {
//             if (data.hits && data.hits.length > 0) {
//               setStart(data.hits[0].name || `${latitude}, ${longitude}`);
//               mapRef.current?.animateToRegion({
//                 latitude,
//                 longitude,
//                 latitudeDelta: 0.05,
