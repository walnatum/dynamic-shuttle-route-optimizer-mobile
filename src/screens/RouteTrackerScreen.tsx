...
const RouteTrackerScreen = ({ navigation }: { navigation: any }) => {
  const mapRef = useRef<MapView>(null);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const [permissionGranted, setPermissionGranted] = useState(false);
  const [studentCode, setStudentCode] = useState("");
  ...
  const updateShuttleDisplay = (studentData: Student) => {
    try {
      ...
      // Store and show pickup location
      if (studentData.point_latitude && studentData.point_longitude && 
          !isNaN(studentData.point_latitude) && !isNaN(studentData.point_longitude)) {
        const pickupLoc = {
          latitude: studentData.point_latitude,
  ...
  return (
    <View style={styles.container}>
        ...
        <Icon
          name="crosshairs-gps"  // Or "navigate" or "map"
          size={24}
          color="#2563EB"
          style={styles.headerIcon}
        />
        <Text style={styles.customHeaderText}>RouteWise - Route Tracker</Text>
      </View>
      {/* Map */}
      <MapView
        ref={mapRef}
        ...
        <>
          ...
          <View style={styles.statusOverlay}>
            ...
            {eta ? (
              ...
              <Text style={styles.statusText}>Student Offboard</Text>
            )}
          </View>
        </>
      )}
